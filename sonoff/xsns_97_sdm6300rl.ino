...
/*********************************************************************************************\
 * Eastron SDM630-Modbus RL energy meter
 *
...
float sdm630_power_thdltn = 0;
float sdm630_energy_total = 0;
float sdm630_energy_sum = 0;
...
const uint16_t sdm630_start_addresses[] {
  ...
  0x00F8, //Average line to neutral volts THD SDM630_POWER_THDLTN [V]
...
void SDM630Show(bool json)
{
  ...
  dtostrfd(sdm630_voltage[2], 1, voltage_l3);
  ...
  if (json) {
    ResponseAppend_P(PSTR(",\"" D_RSLT_ENERGY "\":{\"" D_JSON_TOTAL "\":%s,\""
      ...
      voltage_l1, voltage_l2, voltage_l3, voltage_thdltn,
    ...
    if (0 == tele_period) {
      ...
      switch (i) {
        ...
        //{"idx":626,"nvalue":0,"svalue":"732741","Battery":200,"RSSI":10}
           ...
           memcpy(data, mqtt_data, sizeof(data));
           Response_P(DOMOTICZ_MESSAGE, DZ_FACTOR_IDX+2, 0, power_factor_l3, batq, rssi);
           MqttPublish(domoticz_in_topic);
           memcpy(mqtt_data, data, sizeof(data));
           delayMicroseconds(500);
      ...
      DomoticzSensor(DZ_POWER_ENERGY, energy_total);
    }
#endif  // USE_DOMOTICZ
